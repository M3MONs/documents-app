...
const ContentItem = memo(({ item, canManageCategory, onItemClick, onManageClick, onDeleteClick }: ContentItemProps) => {
      ...
      <CardContent className="flex items-center justify-between py-3 px-4">
        <div className="flex items-center gap-3 flex-1 min-w-0">
          ...
          <div className="flex-1 min-w-0">
            ...
            {!isFolder && item.mime_type && <p className="text-xs text-muted-foreground">{item.mime_type}</p>}
          </div>
        </div>
        {canManageCategory && (
          <DropdownMenu>
