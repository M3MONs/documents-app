...
import { getDepartmentsColumns } from "./departmentsColumns.tsx";
...
const DepartmentsTab: React.FC<DepartmentsTabProps> = ({ selectedFolder }) => {
    ...
    const refreshData = () => {
        queryClient.invalidateQueries({ queryKey: ["admin/folder-departments"] });
    };

    const { data: departmentsData, isLoading: departmentsLoading } = useQuery({
        queryKey: ["admin/folder-departments", selectedFolder?.id || "", pageIndex, pageSize, sorting, columnFilters],
        queryFn: () => {
            ...
            return AdminService.getFolderDepartments(selectedFolder.id, pagination);
        },
    ...
    const handleAssignAction = async (department: Department) => {
        ...
        try {
            await AdminService.assignDepartmentToFolder(selectedFolder.id, department.id);
    ...
    const handleUnassignAction = async (department: Department) => {
        if (!selectedFolder) return;
        try {
            await AdminService.unassignDepartmentFromFolder(selectedFolder.id, department.id);
